...
class Properties(Singleton):
    ...
    def load_file(self, filename):
        ...
        with open(filename, 'U') as f:
            lines_read = f.read()
            self._textfile = lines_read     # store raw file
            lines = lines_read.split('\n')
...
##        fd.close()
    ...
    def Validate(self):
        ...
        # if object_table field defined then object_id and cell_x_loc and cell_y_loc should be defined
        if self.field_defined('object_table'):
            assert self.field_defined('object_id') and self.field_defined('cell_y_loc') and self.field_defined('cell_x_loc'), 'PROPERTIES ERROR (cell location): cell_x_loc and cell_y_loc should be defined when there is an object table!'
